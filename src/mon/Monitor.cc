...
int Monitor::scrub_start()
{
  ...
  if (scrub_con.load() != nullptr) {
    ...
    if (local_con) {
      if (!local_con->scrub_result.empty()) {
        clog->info() << "scrub already in progress";
        return -EBUSY;
      }
      local_con->scrub_result.clear();
    }
  }

  scrub_event_cancel();
  scrub_con.store(std::make_shared<ScrubContext>());
...
int Monitor::scrub()
{
  ...
  std::shared_ptr<ScrubContext> local_con = scrub_con;
  if (nullptr == local_con)
    return -1; // scrub aborted

  ceph_assert(local_con->scrub_state);
  local_con->scrub_version = paxos->get_version();
  ...
  for (set<int>::iterator p = quorum.begin();
    ...
    if (*p == rank)
      continue;
    MMonScrub *r = new MMonScrub(MMonScrub::OP_SCRUB, local_con->scrub_version,
                                 num_keys);
    r->key = local_con->scrub_state->last_key;
    send_mon_message(r, *p);
  }

  // scrub my keys
  bool r = _scrub(&local_con->scrub_result[rank],
                  &local_con->scrub_state->last_key,
                  &num_keys);

  local_con->scrub_state->finished = !r;
  ...
  if (quorum.size() == 1) {
    ceph_assert(local_con->scrub_state->finished == true);
    scrub_finish();
  }
  return 0;
}
...
void Monitor::handle_scrub(MonOpRequestRef op)
{
  ...
  case MMonScrub::OP_RESULT:
	...
	break;
      ...
      if (m->version != local_con->scrub_version)
	break;
      // reset the timeout each time we get a result
      scrub_reset_timeout();

      int from = m->get_source().num();
      ceph_assert(local_con->scrub_result.count(from) == 0);
      local_con->scrub_result[from] = m->result;

      if (local_con->scrub_result.size() == quorum.size()) {
        scrub_check_results();
        local_con->scrub_result.clear();
        if (local_con->scrub_state->finished)
          scrub_finish();
        else
          scrub();
      }
...
void Monitor::scrub_check_results()
{
  ...
  if (nullptr == local_con) {
    ++errors;
    clog->error() << "scrub check_result skipped due to invalidate scrub context";
    return;
  }

  ScrubResult& mine = local_con->scrub_result[rank];
  for (map<int,ScrubResult>::iterator p = local_con->scrub_result.begin();
       p != local_con->scrub_result.end();
       ++p) {
    if (p->first == rank)
      continue;
    if (p->second != mine) {
...
inline void Monitor::scrub_update_interval(ceph::timespan interval)
{
  ...
  std::shared_ptr<ScrubContext> local_con = scrub_con;
  if (local_con && local_con->scrub_state)
    return;
