  ...
  ceph::Formatter* f) const
{
  ...
  if (auto it = pg_pool_sum.find(-1); it != pg_pool_sum.end()) {
  ...
  if (!stray_pool_sum.empty() || meta_pool_stats) {
    if (f) {
      ...
      for (auto& [pool, statfs] : stray_pool_sum) {
        f->open_object_section("pool_stat");
        f->dump_string("pool", "<wiping>");
        f->dump_int("poolid", pool);
        f->dump_int("total_used", statfs.allocated);
...
void PGMap::apply_incremental(CephContext *cct, const Incremental& inc)
{
  ...
  for (auto p = inc.pool_statfs_updates.begin();
    ...
    if (update_pool == -1 || pg_pool_sum.count(update_pool)) {
      pool_stat_t &pool_sum_ref = pg_pool_sum[update_pool];
  ...
  // incremental
  for (auto& [osd, per_pool_statfs] : stray_statfs_inc) {
    for (auto& [pool, stats] : per_pool_statfs) {
      stray_pool_sum[pool].add(stats);
    }
    stray_pool_statfs[osd].swap(per_pool_statfs);
    reported_osds.erase(osd);
    for (auto& [pool, stats] : per_pool_statfs) {
  ...
  // sanity print
  if (cct && cct->_conf->subsys.should_gather<ceph_subsys_osd, 20>()) {
    size_t count = 0;
