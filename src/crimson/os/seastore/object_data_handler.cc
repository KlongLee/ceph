  ...
  extent_to_insert_list_t &to_insert)
{
  return trans_intr::do_for_each(
	...
	return ctx.tm.alloc_data_extents<ObjectDataBlock>(
	  ctx.t,
	  laddr_hint_t::build_never_collide_hint(region.addr, ctx.t.get_offset_bits()),
	  region.len
  ...
  extent_len_t size)
{
  ...
  if (!object_data.is_null()) {
    ...
    auto hint = ctx.onode.get_data_hint(ctx.t.get_offset_bits());
    DEBUGT("reserving: {}~{}",
           ctx.t,
           hint,
           max_object_size);
	...
	return trans_intr::do_for_each(
	  ...
	  if (pin->get_val().is_zero()) {
	    fut = ctx.tm.reserve_region(
	      ctx.t,
	      laddr_hint_t::build_never_collide_hint(addr, ctx.t.get_offset_bits()),
	      pin->get_length());
	  } else {
	  ...
	  if (last_pos != object_data.get_reserved_data_len()) {
	    ...
	    return ctx.tm.reserve_region(
	      ctx.t,
	      laddr_hint_t::build_never_collide_hint(laddr, ctx.t.get_offset_bits()),
