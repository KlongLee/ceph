  ...
  std::list<BtreeLBAMappingRef> &pin_list)
{
  return seastar::do_with(
	...
	auto offset_bits = c.trans.get_offset_bits();
	auto intermediate_key = pin->get_raw_val()
	    .build_laddr(key, offset_bits);
  ...
  bool cascade_remove)
{
  ...
  return _update_mapping(
	...
	t,
	map_value.pladdr.build_laddr(
	  addr, t.get_offset_bits()),
	map_value.len
      );
    }
    ...
    std::vector<alloc_mapping_info_t> &alloc_infos)
{
  ...
  if (alloc_infos.front().key == L_ADDR_NULL) {
    ...
    ceph_assert(hint.never_conflict());
  ...
  while (true) {
    ...
    if (lookup_attempts / threshold == next_alarm) {
      ...
      WARNT("attempt searching lba btree over {} times -- hint: {} cur hint: {} {}",
	    t, lookup_attempts, orig_hint, hint, laddr_printer_t{orig_hint.addr});
    }
    ...
    if (iter.is_end()) {
      ...
      co_return insert_pos_t(iter, hint.addr);
	       ...
	       key >= hint.addr + length && !hint.collide_with(key)) {
      ...
      if (hint.linear_search_on_collision()) {
