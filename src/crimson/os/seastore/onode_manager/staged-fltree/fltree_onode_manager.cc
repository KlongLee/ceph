  ...
  laddr_offset_t value_addr_offset)
{
  ...
  try {
    ...
    case delta_op_t::UPDATE_LOCAL_OBJECT_ID:
      ...
      break;
    case delta_op_t::UPDATE_OFFSET_BITS:
      DEBUG("update offset bits");
      bliter.copy(sizeof(mlayout.offset_bits), (char *)&mlayout.offset_bits);
      break;
  ...
  NodeExtentMutable &payload_mut, delta_op_t op)
{
  ...
  case delta_op_t::UPDATE_OFFSET_BITS:
    DEBUG("update offset bits");
    encoded.append(
      (const char *)&layout.offset_bits,
      sizeof(layout.offset_bits));
    break;
  case delta_op_t::UPDATE_OBJECT_DATA:
    DEBUG("update object data");
    encoded.append(
      ...
      return tree.lower_bound(trans, start
		    ...
		    cursor.value());
                  ...
                  if (!obj_data.is_null()) {
                    ...
                    if (prefix != L_ADDR_NULL && !obj_data.is_null()) {
                      assert(prefix == obj_data.get_reserved_data_base().get_prefix());
                    }
                  }
                }
		...
		snap.reset(new FLTreeOnode(std::move(onode)));
                if (prefix == L_ADDR_NULL) {
                  prefix = obj_data.get_reserved_data_base().get_prefix();
                } else {
                  assert(prefix == obj_data.get_reserved_data_base().get_prefix());
                }
	      } else {
