...
TEST_F(TestSSEKMS, test_transit_create_bucket_key){
  cct->_conf.set_val("rgw_crypt_sse_s3_vault_key_allow_rotation", "true");
  ...
  // Mocks the expected return Value from Vault Server using custom Argument Action
  string post_data = R"({"type":"chacha20-poly1305","derived":true,"allow_rotation":true,"auto_rotate_period":"86400"})";
