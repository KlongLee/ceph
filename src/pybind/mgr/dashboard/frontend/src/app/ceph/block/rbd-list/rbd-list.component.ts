 ...
 messages = { 
  ...
  constructor(
    ...
    const copyAction: CdTableAction = {
      permission: 'create',
      canBePrimary: (selection: CdTableSelection) => selection.hasSingleSelection,
      disable: (selection: CdTableSelection) => this.getCopyDisableDesc(selection),
      icon: Icons.copy,
  ...
  getCopyDisableDesc(selection:CdTableSelection):string | boolean { 
        const first = selection.first();
        if( first && first.mirror_mode === 'Disabled'){
          return `Mirroring needs to be enabled on the image to perform this action`
        }

        return this.getRemovingStatusDesc(selection) || this.getInvalidNameDisable(selection) || !!selection.first().cdExecuting || selection.first().mirror_mode === 'Disabled';
  ...
  getDeleteDisableDesc(selection: CdTableSelection): string | boolean {
    ...
    if (first && !this.imageIsPrimary(first)) {
      return $localize`The image cannot be deleted as it is not primary`;
    }
    return this.getInvalidNameDisable(selection) || this.hasClonedSnapshots(selection.first()) || !this.imageIsPrimary(selection.first());
  }
